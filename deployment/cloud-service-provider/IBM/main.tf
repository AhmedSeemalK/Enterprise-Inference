...
data "template_file" "inference_config" {
  ...
  vars = {
    ...
    deploy_keycloak_and_apisix = var.deploy_keycloak_and_apisix
...
resource "null_resource" "run_script" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "/tmp/run_script.sh '${var.github_pat}' '${ibm_is_instance.vsi3.primary_network_interface[0].primary_ipv4_address}'"
    ]
  }
}
